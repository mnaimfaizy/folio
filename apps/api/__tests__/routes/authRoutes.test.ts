...
jest.mock('../../controllers/authController', () => ({
  ...
  logoutAll: jest.fn((req, res) =>
    res.status(200).json({ message: 'Mocked logout all success' }),
  ),
  refreshSession: jest.fn((req, res) =>
    res.status(200).json({ message: 'Mocked refresh success' }),
  ),
...
describe('Auth Routes', () => {
  ...
  describe('POST /api/auth/logout-all', () => {
    it('should route to logout-all controller and use authentication middleware', async () => {
      const response = await request(app).post('/api/auth/logout-all').send();

      expect(authenticate).toHaveBeenCalled();
      expect(response.status).toBe(200);
      expect(response.body.message).toBe('Mocked logout all success');
    });
  });

  describe('POST /api/auth/refresh', () => {
    it('should route to refresh session controller', async () => {
      const response = await request(app)
        .post('/api/auth/refresh')
        .send({ refreshToken: 'test-refresh-token' });

      expect(response.status).toBe(200);
      expect(response.body.message).toBe('Mocked refresh success');
    });
  });
